...
const recreateRepoTableNoCounterpartyType = async () => {
  try {
    console.log('🚀 Recreating repo_deals table without counterparty_type field...');

    // Drop existing table
    await db.query('DROP TABLE IF EXISTS repo_deals');
    console.log('✅ Dropped existing repo_deals table');

    // Create new table without counterparty_type
    const sql = `
    ...
    await db.query(sql);
    console.log('✅ repo_deals table recreated successfully without counterparty_type field');
    ...
    console.error('❌ Error recreating repo_deals table:', error);
    throw error;
  }
};
...
if (require.main === module) {
  recreateRepoTableNoCounterpartyType()
    .then(() => {
...
module.exports = recreateRepoTableNoCounterpartyType;
