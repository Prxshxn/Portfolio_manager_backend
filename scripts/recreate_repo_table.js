...
const recreateRepoTable = async () => {
  try {
    console.log('🚀 Recreating repo_deals table with new structure...');

    // Drop existing table
    await db.query('DROP TABLE IF EXISTS repo_deals');
    console.log('✅ Dropped existing repo_deals table');

    // Create new table with counterparty_type
    const sql = `
    ...
    await db.query(sql);
    console.log('✅ repo_deals table recreated successfully with new structure');
    ...
    console.log('🎉 repo_deals table recreated successfully!');
...
module.exports = recreateRepoTable;
