 ...
 * to its own expense account "Intrest Expense R/Repo" (669-101-240-768-44),
...
async function run() {
  ...
  if (!rows.length) {
    console.error(
      `Missing chart_of_accounts row for ${CODE}. Add "Intrest Expense R/Repo" to the chart, then re-run.`
    );
    process.exit(1);
  }
  ...
  await db.query(
    `INSERT INTO account_mappings (mapping_key, account_code, description, is_active, created_at, updated_at)
     ...
     ON DUPLICATE KEY UPDATE
       ...
       is_active = TRUE,
       updated_at = NOW()`,
    [KEY, CODE, 'Intrest Expense R/Repo (reverse repo maturity interest expense)']
  );
